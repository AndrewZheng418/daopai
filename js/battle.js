...
async function init() {
  ...
  document.getElementById('btn-menu').addEventListener('click', confirmReturnMenu);
  ...
  document.getElementById('btn-close-mobile-log').addEventListener('click', toggleMobileLog);
  ...
  document.getElementById('btn-im-menu').addEventListener('click', confirmReturnMenu);
...
function render() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    seat.classList.toggle('active', pokerGame.currentPlayer === p && !p.folded && !p.allIn);
  ...
  if (hp) {
    ...
    document.getElementById('playerBet').textContent = hp.currentBet > 0 ? `本轮已注: ${hp.currentBet.toLocaleString()}` : '本轮未注';
...
function scheduleAI() {
  ...
  if (diff === 'hard') {
    const sayTrash = Math.random() < 0.3;
    if (sayTrash) {
      const trashLines = [
        '「这点气运，也敢与我争锋？」',
        '「你的牌路，我已经看穿了。」',
      ...
      const line = trashLines[Math.floor(Math.random() * trashLines.length)];
...
function onHandEnded() {
  ...
  if (saveData.round >= (saveData.totalRounds || 5)) {
    endGame(true);
    return;
  }
...
function showHandResult() {
  ...
  // 亮出所有未遁走玩家的手牌 + 最佳五张
  const activePlayers = pokerGame.players.filter(p => !p.folded && !p.eliminated);
  ...
  activePlayers.forEach(p => {
    ...
    html += `<div class="reveal-card ${wonAmt > 0 ? 'winner' : ''}">
      <div class="rc-name">${p.name} ${p.isHuman ? '（你）' : ''}${wonAmt > 0 ? '<span style="margin-left:6px; color:#4a6a4a; font-size:11px;">🏆 +' + wonAmt.toLocaleString() + '</span>' : ''}</div>
...
function toggleMobileLog() {
  document.getElementById('mobileLogModal').classList.toggle('show');
}
